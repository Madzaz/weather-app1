$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $("#cityName2").html("&#8630 " + response["location"]["name"]);
  ...
  function getWeatherData(city, containerId) {
    $.ajax({
      method: "GET",
      url: "http://api.weatherapi.com/v1/forecast.json",
      data: {
      ...
      success: function (response) {
        $(`.${containerId} #countryTemp`).text(
          response["current"]["temp_c"] + "°"
        );

        var todayForecast = response.forecast.forecastday[0];
        $(`.${containerId} #countryHigh`).text(
          "H:" + todayForecast["day"]["maxtemp_c"] + "°"
        );
        $(`.${containerId} #countryLow`).text(
          "L:" + todayForecast["day"]["mintemp_c"] + "°"
        );

        $(`.${containerId} #countryName`).text(
          response["location"]["name"] + ", " + response["location"]["country"]
        );

        $(`.${containerId} #countryWeather`).attr(
          "src",
  ...
  getWeatherData("Leskovac", "firstCity");
  getWeatherData("Novi Sad", "secondCity");
  getWeatherData("Zrenjanin", "thirdCity");
});
